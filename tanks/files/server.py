...
class GameServer:
    ...
    def update_players(self, dt):
        for player_id, player in self.players.items():
            ...
            mouse_steer = bool(controls.get("mouse_steer"))

            if mouse_steer:
                new_rot = self.turn_toward(new_rot, controls.get("aim", new_rot), 120 * dt)
            else:
    ...
    def update_spawns(self, now):
        ...
        if now >= self.next_scrap_spawn:
            if random.random() < 0.7:
                self.spawn_scrap()
            original_interval = max(1.5, random.uniform(5, 15) / (scrap_multiplier * 2.0))
                    ...
                    with self.lock:
                        if message.get("type") == "input" and player_id in self.inputs:
                            self.inputs[player_id] = {
                                ...
                                "mouse_steer": bool(message.get("mouse_steer", False))
                            }
