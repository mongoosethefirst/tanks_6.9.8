    ...
    def run(self):
        ...
        while self.running:
            ...
            if input_timer >= 1 / 30 and not over and state.get("match_started", False):
                send_json(self.sock, {
                    "type": "input",
                    "left": (keys[pygame.K_a] or keys[pygame.K_LEFT]) and not mouse_steer,
                    "right": (keys[pygame.K_d] or keys[pygame.K_RIGHT]) and not mouse_steer,
                    "forward": keys[pygame.K_w] or keys[pygame.K_UP],
                    "backward": keys[pygame.K_s] or keys[pygame.K_DOWN],
                    "aim": target,
                    "mouse_steer": mouse_steer
                })
                input_timer = 0
            ...
            self.txt(screen, small, "MOUSE STEER: " + ("ON" if mouse_steer else "OFF") + " [M]", (55, height - 18), (220, 220, 220))
